...
func (s *Suite) TestNsm_istio() {
	...
	r.Run(`kubectl --kubeconfig=$KUBECONFIG1 apply -f https://github.com/kubernetes-sigs/metrics-server/releases/latest/download/components.yaml`)
	r.Run(`kubectl top pods -A`)
	r.Run(`kubectl top nodes -A`)
	r.Run(`kubectl --kubeconfig=$KUBECONFIG2 apply -f https://github.com/kubernetes-sigs/metrics-server/releases/latest/download/components.yaml`)
	r.Run(`kubectl top pods -A`)
	r.Run(`kubectl top nodes -A`)
